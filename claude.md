...
6. **Đồng bộ Git:** Luôn kết thúc một task quan trọng bằng việc tạo commit rõ ràng và push lên repository. **BẮT BUỘC luôn dùng lệnh `git add .`** (thay vì add từng file cụ thể) để đảm bảo không bỏ sót bất kỳ file nào (kể cả file cấu hình, script hay file phụ trợ). Sau đó mới thực hiện commit và push.
