...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        decision/.style={diamond, aspect=2, text centered, text width=2cm, draw=black, fill=orange!20},
        ...
        % Split to excess and triage
        \draw [thick] (step2.south) -- ++(0,-0.3) coordinate (split);
        \draw [arrow] (split) -- (triage.north);
        \draw [arrow] (split) -| (excess.north);
        
        % Triage to levels
        \draw [arrow] (triage.south) -- node[left] {Adequate} (adeq.north);
        \draw [arrow] (triage.west) -- node[above] {Critical} (crit.east);
        \draw [arrow] (triage.east) -- node[above] {Low} (low.west);
        
        % Levels to context
        \draw [arrow] (adeq.south) -- (context.north);
        \draw [arrow] (low.south) |- (context.east);
        
        % Context to def_on / def_off
        \draw [arrow] (context.south) -- node[left] {Đủ ĐK} (def_on.north);
        \draw [arrow] (context.west) -- node[above] {Chưa đủ} (def_off.east);
        
        % def_on -> merge
        \draw [arrow] (def_on.south) -- (merge.north);
        
        % crit -> merge (bypassing context)
        \draw [arrow] (crit.west) -- ++(-0.5,0) |- (merge.west);
        
        % def_off -> merge
        \draw [arrow] (def_off.south) |- (merge.west);
        
        % excess -> merge
        \draw [arrow] (excess.east) -- ++(0.5,0) |- (merge.east);
        
        % merge -> insight
        \draw [arrow] (merge.south) -- (insight.north);
