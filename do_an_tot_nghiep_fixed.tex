...
% Cột
\node[font=\bfseries] (uTitle) at (0,0) {Người dùng};
\node[font=\bfseries] (feTitle) at (4,0) {Frontend};
\node[font=\bfseries] (beTitle) at (8.5,0) {Auth/Middleware};
\node[font=\bfseries] (svcTitle) at (13,0) {Controller/Service};

% Đường phân cách cột
\draw[dashed, gray] (2,-0.5) -- (2,-11.5);
\draw[dashed, gray] (6.3,-0.5) -- (6.3,-11.5);
\draw[dashed, gray] (10.8,-0.5) -- (10.8,-11.5);

% Các nút chính
\node[box] (loginInput) at (0,-1.3) {Nhập email\\mật khẩu};
\node[box] (loginReq) at (4,-2.3) {POST\\{\ttfamily /api/v1/auth/login}};
\node[box] (checkLogin) at (8.5,-3.3) {Kiểm tra tài khoản\\và mật khẩu};
\node[box] (tokenReturn) at (8.5,-4.5) {Tạo Access Token\\và Refresh Token};
\node[box] (saveToken) at (4,-5.7) {Nhận Access Token\\Refresh Token ở Cookie};

\node[box] (apiReq) at (4,-7.0) {Gọi API nghiệp vụ\\Authorization: Bearer Token};
\node[box] (checkJwt) at (8.5,-8.0) {Middleware\\kiểm tra JWT};

\node[box] (business) at (13,-8.9) {Xử lý nghiệp vụ\\Controller/Service};
\node[box] (successJson) at (4,-10.0) {Nhận Response JSON\\và cập nhật giao diện};

% Nhánh refresh token
\node[smallbox] (expired) at (8.5,-9.7) {Token hết hạn\\trả 401 JSON};
\node[smallbox] (refreshReq) at (4,-11.0) {POST\\{\ttfamily /auth/refresh-token}};
\node[smallbox] (checkRefresh) at (8.5,-12.2) {Kiểm tra Refresh Token\\trong HttpOnly Cookie};
\node[smallbox] (newToken) at (4,-13.4) {Nhận Access Token mới\\gọi lại request cũ};

% Mũi tên luồng đăng nhập
\draw[arrow] (loginInput) -- (loginReq);
\draw[arrow] (loginReq) -- (checkLogin);
\draw[arrow] (checkLogin) -- (tokenReturn);
\draw[arrow] (tokenReturn) -- node[above, font=\small] {JSON + Cookie} (saveToken);
...
% Nhánh hợp lệ
\draw[arrow] (checkJwt) -- node[above, font=\small] {Hợp lệ} (business);
\draw[arrow] (business) -- node[below, font=\small] {Response JSON} (successJson);

% Nhánh hết hạn
\draw[dashedarrow] (checkJwt) -- node[right, font=\small] {Hết hạn} (expired);
\draw[dashedarrow] (expired) -- (refreshReq);
\draw[dashedarrow] (refreshReq) -- (checkRefresh);
\draw[dashedarrow] (checkRefresh) -- node[above, font=\small] {Access Token mới} (newToken);
\draw[dashedarrow] (newToken) to[out=160,in=200] (apiReq);

% Chú thích
\node[note] at (6.5,-14.3) {
Đường liền: luồng xác thực và gọi API thông thường.\\
Đường nét đứt: luồng làm mới Access Token khi token truy cập hết hạn.
};
...
\hline
\endfirsthead
\caption[]{Các trạng thái chính của log Adaptive TDEE (tiếp theo)} \\
\hline
\textbf{Trạng thái} & \textbf{Ý nghĩa} & \textbf{Ảnh hưởng} \\
\hline
\endhead
