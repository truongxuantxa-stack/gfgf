...
\begin{figure}[htbp]
    ...
    % Cần dùng thư viện positioning
    % BACKEND
    \node[box] (api) {\textbf{RESTful API Route}\\[1ex] \scriptsize \texttt{/api/v1/auth}, \texttt{/api/v1/diary/entries},\\[0.5ex] \texttt{/api/v1/dashboard}, ...};
    \node[box, below=0.8cm of api] (middleware) {\textbf{Middleware xác thực}\\[1ex] \scriptsize Kiểm tra Access Token / JWT\\Nếu sai $\rightarrow$ trả 401 JSON};
    \node[box, below=0.8cm of middleware] (controller) {\textbf{Controller}\\[1ex] \scriptsize Nhận request, gọi service, trả response};
    \node[box, below=0.8cm of controller] (service) {\textbf{Service}\\[1ex] \scriptsize Xử lý nghiệp vụ, tính toán, tổng hợp dữ liệu};

    % FRONTEND
    \node[box, left=2.5cm of middleware, yshift=-1.1cm] (axios) {\textbf{Axios / TanStack Query}\\[1ex] \scriptsize Gửi HTTP request, quản lý cache, invalidate query};
    \node[box, fill=blue!10, above=0.8cm of axios] (react) {\textbf{React Frontend}\\[1ex] \scriptsize Pages, Components, Forms, Modals};

    % DATABASE
    \node[box, right=2.5cm of controller] (model) {\textbf{Sequelize Model}};
    \node[box, right=2.5cm of service] (db) {\textbf{MySQL Database}};

    % Titles
    \node[font=\large\bfseries\sffamily, text=black, above=1.2cm of react] (title_fe) {FRONTEND};
    \node[font=\large\bfseries\sffamily, text=black] at (title_fe -| api) (title_be) {BACKEND};
    \node[font=\large\bfseries\sffamily, text=black] at (title_fe -| model) (title_db) {DATABASE};

    % Separators
    \path (react.east) -- (api.west) coordinate[midway] (sep1);
    \path (api.east) -- (model.west) coordinate[midway] (sep2);
    \draw[thick, dashed, gray!50] (sep1 |- title_fe.north) -- (sep1 |- service.south);
    \draw[thick, dashed, gray!50] (sep2 |- title_be.north) -- (sep2 |- service.south);

    % Request Flows (Blue)
    \draw[req] ([xshift=-0.5cm]react.south) -- ([xshift=-0.5cm]axios.north);
    
    % HTTP Request: label on first horizontal segment
    \draw[req] ([yshift=0.2cm]axios.east) -- node[label_req, above] {HTTP Request} ++(1.5,0) |- (api.west);
    
    % Backend internal requests (straight down)
    \draw[req] ([xshift=-0.5cm]api.south) -- ([xshift=-0.5cm]middleware.north);
    \draw[req] ([xshift=-0.5cm]middleware.south) -- ([xshift=-0.5cm]controller.north);
    \draw[req] ([xshift=-0.5cm]controller.south) -- ([xshift=-0.5cm]service.north);
    
    % Backend to DB
    \draw[req] ([yshift=0.2cm]service.east) -- node[label_req, above] {Query} ++(1.2,0) |- ([yshift=0.2cm]model.west);
    ...
    \draw[res] ([yshift=-0.2cm]model.west) -- node[label_res, below] {Data} ++(-0.8,0) |- ([yshift=-0.2cm]service.east);
    
    % Backend internal responses (straight up)
    \draw[res] ([xshift=0.5cm]service.north) -- node[label_res, right] {Data} ([xshift=0.5cm]controller.south);
    \draw[res] ([xshift=0.5cm]controller.north) -- ([xshift=0.5cm]middleware.south);
    \draw[res] ([xshift=0.5cm]middleware.north) -- ([xshift=0.5cm]api.south);
    
    % Response JSON to Axios
    \draw[res] ([yshift=-0.2cm]api.west) -- node[label_res, above] {Response JSON} ++(-1.5,0) |- ([yshift=-0.2cm]axios.east);

    % 401 Error flow to Axios
    \draw[res, thick, red] (middleware.west) -- node[label_res, above] {401 Error} ++(-1.0,0) |- ([yshift=0.6cm]axios.east);
