...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw, rounded corners, minimum width=2.6cm, minimum height=1.2cm, align=center, fill=blue!5, font=\small\sffamily},
        db/.style={rectangle, draw, rounded corners, minimum width=2cm, minimum height=2.5cm, align=center, fill=gray!10, font=\small\sffamily},
        arrow/.style={->, >=stealth, thick},
        dashed_arrow/.style={->, >=stealth, thick, dashed}
        ]

        % Nodes
        \node[box, fill=green!5] (router) {React Router \& Pages\\(Lớp UI)};
        \node[box, below=1.5cm of router, fill=yellow!10] (hooks) {Custom Hooks\\(Lớp Nghiệp vụ)};
        
        \node[box, right=1.5cm of router, fill=orange!5] (auth) {Auth Context\\(Quản lý Đăng nhập)};
        \node[box, right=1.5cm of hooks, fill=cyan!5] (reactquery) {TanStack Query\\(Quản lý State \& Cache)};
        
        \node[box, right=1.5cm of reactquery, fill=purple!5] (axios) {Axios Instance\\(HTTP Client)};
        
        \node[db, right=1.5cm of axios] (backend) {Backend API\\(Express)};

        % Background for Frontend
        \begin{pgfonlayer}{background}
            \node[rectangle, draw, dashed, fill=blue!2, rounded corners, fit=(router) (hooks) (auth) (reactquery) (axios), inner sep=0.6cm] (frontend) {};
        ...
        % Arrows
        % Router <-> Hooks
        \draw[arrow] ([xshift=2mm]router.south) -- node[right, font=\scriptsize] {Gọi} ([xshift=2mm]hooks.north);
        \draw[arrow] ([xshift=-2mm]hooks.north) -- node[left, font=\scriptsize] {Dữ liệu \& UI State} ([xshift=-2mm]router.south);

        % Router -> Auth Context
        \draw[dashed_arrow] (router.east) -- node[above, font=\scriptsize] {Kiểm tra User/Token} (auth.west);
        
        % Hooks <-> TanStack Query
        \draw[arrow] ([yshift=2mm]hooks.east) -- node[above, font=\scriptsize] {Sử dụng useQuery/...} ([yshift=2mm]reactquery.west);
        \draw[arrow] ([yshift=-2mm]reactquery.west) -- node[below, font=\scriptsize] {Trả Data/Loading/Error} ([yshift=-2mm]hooks.east);
        
        % TanStack Query <-> Axios
        \draw[arrow] ([yshift=2mm]reactquery.east) -- node[above, font=\scriptsize] {Fetch/Mutate} ([yshift=2mm]axios.west);
        \draw[arrow] ([yshift=-2mm]axios.west) -- node[below, font=\scriptsize] {Promise} ([yshift=-2mm]reactquery.east);
        
        % Axios -> Auth (Interceptor)
        \draw[dashed_arrow] (axios.north) |- node[pos=0.75, above, font=\scriptsize] {Lấy Token gắn vào Header} (auth.east);
        
        % Axios <-> Backend
        \draw[arrow] ([yshift=3mm]axios.east) -- node[above, font=\scriptsize] {HTTP Request} ([yshift=3mm]backend.west);
        \draw[arrow] ([yshift=-3mm]backend.west) -- node[below, font=\scriptsize] {JSON Response} ([yshift=-3mm]axios.east);
