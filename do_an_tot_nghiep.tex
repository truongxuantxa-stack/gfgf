...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw=black, thick, fill=blue!5, rounded corners=4pt,
                    text width=4cm, align=center, font=\small\sffamily, inner sep=5pt,
                    minimum height=1.2cm},
        storebox/.style={rectangle, draw=gray!60, thick, fill=gray!8, rounded corners=3pt,
                    text width=3.5cm, align=center, font=\scriptsize\sffamily, inner sep=4pt, dashed},
        dbbox/.style={rectangle, draw=black, thick, fill=green!5, rounded corners=4pt,
                    text width=3.5cm, align=center, font=\small\sffamily, inner sep=5pt,
                    minimum height=1cm},
    ...
    % ═══════════════ FRONTEND (cột trái, cách xa 4cm) ═══════════════
    \node[box, fill=blue!10, left=4cm of api, yshift=-0.5cm] (react)
        {\textbf{React Frontend}\\[0.5ex]
        \scriptsize Pages, Components, Modals};

    \node[box, below=of react] (axios)
        {\textbf{Axios / TanStack Query}\\[0.5ex]
        \scriptsize Gắn Bearer token\\
        Quản lý cache\\
        Interceptor: bắt 401};

    \node[storebox, below=of axios] (ls)
        {\textbf{localStorage}\\[0.3ex]
        \scriptsize Lưu \texttt{accessToken}};

    % ═══════════════ DATABASE (cột phải, cách xa 4cm) ═══════════════
    \node[dbbox, right=4cm of svc] (model)
    ...
    % React → Axios
    \draw[req] ([xshift=-4mm]react.south) -- ([xshift=-4mm]axios.north);
    ...
    % Axios → API Route (đường ngang dài, nhãn ở giữa)
    \draw[req] ([yshift=2mm]axios.east) -- node[lreq, above, midway] {HTTP Request} ([yshift=2mm]api.west);
    ...
    % Svc → Model → DB
    \draw[req] ([yshift=2mm]svc.east) -- node[lreq, above, midway] {Query} ([yshift=2mm]model.west);
    ...
    % Model → Svc
    \draw[res] ([yshift=-2mm]model.west) -- node[lres, below, midway] {Data} ([yshift=-2mm]svc.east);
    ...
    % API → Axios (đường ngang dài, nhãn ở giữa)
    \draw[res] ([yshift=-2mm]api.west) -- node[lres, above, midway] {Response JSON} ([yshift=-2mm]axios.east);

    % Axios → React
    \draw[res] ([xshift=4mm]axios.north) -- ([xshift=4mm]react.south);

    % ═══════════════ LUỒNG 401 (riêng biệt, không đè) ═══════════════
    % MW trả 401 → đi ngang sang Axios (offset y riêng, tránh đè đường khác)
    \draw[err] (mw.west) -- node[lerr, above, midway] {401 JSON} (axios.east |- mw.west);
