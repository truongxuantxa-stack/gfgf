...
Nhóm chức năng thứ sáu là xây dựng thực đơn và xuất báo cáo. Meal Planner cần hỗ trợ tạo gợi ý thực đơn dựa trên mục tiêu dinh dưỡng và dữ liệu thực phẩm. Chức năng báo cáo PDF cần tổng hợp dữ liệu theo khoảng thời gian, giúp người dùng xem lại kết quả theo tuần hoặc tháng.
...
\begin{longtable}{|p{3.5cm}|p{2cm}|p{4.5cm}|p{5cm}|}
...
Trong nhóm dữ liệu đầu vào, mức độ vận động được sử dụng để lựa chọn hệ số hoạt động thể chất khi tính TDEE. Việc đưa trường này vào hồ sơ người dùng giúp hệ thống ánh xạ dữ liệu khai báo của người dùng sang hệ số PAL phục vụ tính toán nhu cầu năng lượng hằng ngày \cite{fao_who_unu_2001}.
...
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[
        box/.style={rectangle, draw, rounded corners, align=center, minimum width=2.4cm, minimum height=0.9cm, fill=blue!5, font=\footnotesize},
        cond/.style={diamond, aspect=2.2, draw, align=center, fill=orange!10, inner sep=1pt, font=\footnotesize},
        start/.style={ellipse, draw, align=center, fill=gray!20, font=\footnotesize, minimum height=0.8cm, inner sep=2pt},
        core/.style={rectangle, draw, rounded corners, align=center, minimum width=2.6cm, minimum height=0.9cm, fill=green!10, thick, font=\footnotesize},
        arrow/.style={-Latex, thick}
    ]
        
        % Hàng 1 (y=0)
        \node[start] (cron) at (0, 0) {Cron Job\\(Hàng tuần)};
        \node[box] (get_data) at (2.8, 0) {Lấy dữ liệu\\tuần trước};
        \node[cond] (check_data) at (6.6, 0) {Đủ dữ liệu?\\($\ge 5$ ngày, $\ge 2$ lần cân)?};
        \node[box] (log_skip) at (10.8, 0) {Lưu log\\(skipped)};
        \node[start] (end1) at (13.2, 0) {Kết thúc};
        ...
        % Hàng 2 (y=-2)
        \node[box] (calc_cal) at (4.2, -2) {Tính calo\\trung bình};
        \node[box] (calc_weight) at (9.0, -2) {Tính cân nặng\\EMA (làm mịn)};
        
        \coordinate (split) at (6.6, -1.1);
        \draw[thick] (check_data.south) -- node[left, font=\scriptsize] {Có} (split);
        \draw[arrow] (split) -| (calc_cal.north);
        \draw[arrow] (split) -| (calc_weight.north);
        
        % Hàng 3 (y=-4)
        \node[box] (calc_tdee) at (6.6, -4) {Tính TDEE\\tuần};
        \node[box] (limit) at (2.5, -4) {Giới hạn biên $\pm 30\%$\\(so với TDEE tĩnh)};
        \node[box] (calc_roll) at (-1.5, -4) {Tính rolling\\TDEE};
        
        \coordinate (join_tdee) at (6.6, -3.1);
        \draw[thick] (calc_cal.south) |- (join_tdee);
        \draw[thick] (calc_weight.south) |- (join_tdee);
        \draw[arrow] (join_tdee) -- (calc_tdee.north);
        
        \draw[arrow] (calc_tdee) -- (limit);
        \draw[arrow] (limit) -- (calc_roll);
        
        % Hàng 4 (y=-6)
        \node[box] (log_success) at (-1.5, -6) {Lưu log\\(AdaptiveTDEELog)};
        \node[cond] (check_qualify) at (2.5, -6) {Đủ điều kiện\\($\ge 2$ tuần)?};
        \node[box] (update_user) at (6.6, -6) {Cập nhật\\User.adaptiveTDEE};
        \node[start] (end2) at (10.6, -6) {Kết thúc};
        
        \draw[arrow] (calc_roll) -- (log_success);
        \draw[arrow] (log_success) -- (check_qualify);
        \draw[arrow] (check_qualify) -- node[above, font=\scriptsize] {Có} (update_user);
        \draw[arrow] (update_user) -- (end2);
        
        \coordinate (c1) at (2.5, -7.2);
        \draw[arrow] (check_qualify.south) -- node[left, font=\scriptsize] {Không} (c1) -| (end2.south);
        
        % Nutrition core (y=-8)
        \node[core] (core) at (6.6, -8.2) {Nutrition Core\\(Đọc TDEE áp dụng)};
        \draw[arrow, dashed] (update_user.south) -- node[right, align=left, font=\scriptsize] {Được áp dụng ở\\request kế tiếp} (core.north);
...
\begin{longtable}{|p{4cm}|p{4cm}|p{7cm}|}
\caption{trình bày các nhóm dữ liệu chính được sử dụng trong Food Scoring.} \\
\hline
...
\begin{longtable}{|p{4cm}|p{4cm}|p{7cm}|}
\caption{trình bày các nhóm dữ liệu đầu vào chính của Health Insights.} \\
\hline
...
\begin{longtable}{|p{5cm}|p{10cm}|}
\caption{trình bày các nhóm xử lý chính trong Meal Planner.} \\
\hline
