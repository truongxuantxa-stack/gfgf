...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        sysbox/.style={rectangle, draw=black, thick, rounded corners=10pt, inner sep=15pt},
        arrow/.style={-, thick},
        ...
        % --- CỘT 1 ---
        \node[usecase] (uc_register) at (-4.2, 0) {Đăng ký tài khoản};
        \node[usecase] (uc_profile) at (-4.2, -1.5) {Khởi tạo / cập nhật\\hồ sơ sức khỏe};
        \node[usecase] (uc_log_water) at (-4.2, -3) {Ghi nhận nước uống};
        \node[usecase] (uc_dashboard) at (-4.2, -4.5) {Xem Dashboard};
        \node[usecase] (uc_report) at (-4.2, -6) {Xuất báo cáo PDF};
        \node[usecase] (uc_meal) at (-4.2, -8.0) {Xây dựng thực đơn};

        % --- CỘT 2 ---
        \node[usecase] (uc_logout) at (0, 0.75) {Đăng xuất};
        \node[usecase] (uc_login) at (0, -0.75) {Đăng nhập};
        \node[usecase] (uc_log_food) at (0, -2.25) {Ghi nhật ký ăn uống};
        \node[usecase] (uc_weight) at (0, -3.75) {Theo dõi cân nặng};
        \node[usecase] (uc_exercise) at (0, -5.25) {Ghi nhận luyện tập};
        \node[usecase] (uc_aggregate) at (0, -6) {Tổng hợp dữ liệu\\dinh dưỡng};
        \node[usecase] (uc_scanner) at (0, -7.25) {Sử dụng Hybrid\\Nutrition Scanner};

        % --- CỘT 3 ---
        \node[usecase] (uc_search) at (4.2, -2.25) {Tìm kiếm thực phẩm};
        \node[usecase] (uc_analysis) at (4.2, -4.5) {Xem phân tích\\dinh dưỡng};
        \node[usecase] (uc_barcode) at (4.2, -6.5) {Quét mã vạch};
        \node[usecase] (uc_ocr) at (4.2, -8.0) {Nhận dạng bảng\\dinh dưỡng};

        % --- SYSTEM BOUNDARY ---
        \node[sysbox, fit=(uc_logout) (uc_meal) (uc_register) (uc_search) (uc_ocr), label={[font=\bfseries, yshift=8pt]north:HỆ THỐNG QUẢN LÝ DINH DƯỠNG CÁ NHÂN}] (system) {};
        
        \node[anchor=south west, font=\itshape\scriptsize] at ([yshift=5pt, xshift=5pt]system.south west) {* Ghi chú: Các chức năng nghiệp vụ (trừ đăng ký) yêu cầu người dùng đã đăng nhập.};

        % --- ACTORS ---
        \node[actor] (user) at (-8.2, -3.6) {Người dùng};
        \pic at ([yshift=0.8cm]user.center) {stick figure};

        \node[actor, draw=black, thick, fill=gray!10, minimum width=2.2cm, minimum height=0.8cm, align=center, rounded corners=5pt] (openfood) at (8.5, -6.5) {<<system>>\\Open Food Facts\\API};
        \node[actor, draw=black, thick, fill=gray!10, minimum width=2.2cm, minimum height=0.8cm, align=center, rounded corners=5pt] (gemini) at (8.5, -8.0) {<<system>>\\Gemini AI\\Vision};
        ...
        \draw[include] (uc_report) -- node[above, font=\scriptsize] {<<include>>} (uc_aggregate);
        \draw[include] (uc_scanner) -- node[above, sloped, font=\scriptsize] {<<include>>} (uc_barcode);
        \draw[include] (uc_scanner) -- node[below, sloped, font=\scriptsize] {<<include>>} (uc_ocr);
        ...
        % --- USER CONNECTIONS ---
        \draw[-, thick] (user.east) -- (-6.5, -3.6);
        \draw[-, thick] (-6.5, 0.75) -- (-6.5, -8.0);

        % Nối vào Cột 1
        \foreach \y in {0, -1.5, -3.0, -4.5, -6.0, -8.0} {
            \draw[-, thick] (-6.5, \y) -- (-5.9, \y);
        }
        % Nối vào Cột 2
        \foreach \y in {0.75, -0.75, -2.25, -3.75, -5.25, -7.25} {
            \draw[-, thick] (-6.5, \y) -- (-1.7, \y);
        }
