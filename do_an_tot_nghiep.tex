...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[smbox, fill=gray!15] (localstorage) at (11.0, -0.8) {localStorage};
        \node[smbox, fill=red!10] (toaster) at (11.0, 0.6) {react-hot-toast\\(Global Error)};
        
        \node[db] (backend) at (15.5, -2.4) {Backend API\\(Express)};
        ...
        % Auth -> localStorage
        \draw[dashed_arrow] (auth.east) -| node[pos=0.25, above, font=\scriptsize] {Lưu Token} (localstorage.north);
        
        % Axios -> localStorage
        \draw[dashed_arrow] (axios.north) -- node[right, font=\scriptsize, align=center] {Đọc Token\\(Interceptor)} (localstorage.south);
        
        % Axios -> Toaster (Báo lỗi Global)
        \draw[dashed_arrow, orange] (axios.north west) to[out=150, in=180] node[left, font=\scriptsize, align=center, xshift=-1mm] {Báo lỗi\\Global} (toaster.west);
