...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        label_1/.style={font=\scriptsize\sffamily, near start, fill=white, inner sep=1pt},
        label_n/.style={font=\scriptsize\sffamily, near end, fill=white, inner sep=1pt},
        label_01/.style={font=\scriptsize\sffamily, near end, fill=white, inner sep=1pt}
    ]
    ...
    % Daily Logs
    \node[table] (weight) at (-4, 2.25) {WeightLog};
    \node[table] (water) at (-4, 0.75) {WaterLog};
    \node[table] (exercise) at (-4, -0.75) {ExerciseLog};
    \node[table] (tdee) at (-4, -2.25) {AdaptiveTDEELog};
    ...
    % Scanner
    \node[table] (scanner) at (8, 0.5) {ScannedProduct};
    \node[table] (contrib) at (8, -1.5) {ProductContribution};

    % User to Logs
    \draw[rel] (user) -- node[label_1] {1} node[label_n] {N} (weight);
    \draw[rel] (user) -- node[label_1] {1} node[label_n] {N} (water);
    \draw[rel] (user) -- node[label_1] {1} node[label_n] {N} (exercise);
    \draw[rel] (user) -- node[label_1] {1} node[label_n] {N} (tdee);

    % Meal Planner
    \draw[rel] (user) -- node[label_1] {1} node[label_01] {0..1} (mealconfig);
    \draw[dashed_rel] (mealtpl) -- node[font=\scriptsize\sffamily, fill=white, inner sep=1pt, above] {Sử dụng} (mealconfig);
    \draw[dashed_rel] (mealtpl) -- node[font=\scriptsize\sffamily, fill=white, inner sep=1pt, right] {Sử dụng} (food);

    % User to Core
    \draw[rel] (user) -- node[label_1] {1} node[label_n] {N} (diary);
    \draw[rel] (user) -- node[label_1] {1} node[label_n, pos=0.7] {N (tùy chọn)} (food);

    % Food to Diary
    \draw[rel] (food) -- node[label_1] {1} node[label_n] {N} (diary);

    % Scanner
    \draw[rel] (food) -- node[label_1] {1} node[label_n, pos=0.75] {N (tùy chọn)} (scanner);
    \draw[rel] (scanner) -- node[label_1] {1} node[label_n] {N} (contrib);
    \draw[rel] (user) -- node[label_1, pos=0.1] {1} node[label_n, pos=0.85] {N} (contrib);
    ...
    \vspace{0.2cm}
    \small \textit{* Ghi chú: Sơ đồ chỉ thể hiện các bảng trung tâm và quan hệ chính. Các trường dữ liệu chi tiết, ràng buộc và ERD đầy đủ được trình bày trong Phụ lục C.}
\end{figure}
