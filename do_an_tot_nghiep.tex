...
% ============================================================
\begin{titlepage}
    \newgeometry{left=3.0cm, right=2.0cm, top=2.0cm, bottom=2.0cm}
    ...
    \begin{center}
        ...
        % Logo trường
        \includegraphics[height=2.5cm]{images/LOGO_DHXD.png}\\[80pt]
...
Trong quá trình học tập tại Trường Đại học Xây dựng Hà Nội, em đã được trang bị các kiến thức nền tảng và chuyên ngành về lập trình, cơ sở dữ liệu, phân tích thiết kế hệ thống, phát triển ứng dụng web và các phương pháp xây dựng phần mềm. Đồ án tốt nghiệp là cơ hội để em tổng hợp, vận dụng và kiểm chứng những kiến thức đã học vào một bài toán cụ thể. Trên cơ sở đó, đề tài \textbf{“Xây dựng hệ thống quản lý dinh dưỡng cá nhân hóa”} được thực hiện nhằm xây dựng một hệ thống hỗ trợ người dùng theo dõi nhật ký dinh dưỡng, quản lý chỉ số cơ thể, phân tích dữ liệu ăn uống và sinh báo cáo theo thời gian.
...
Em xin gửi lời cảm ơn chân thành đến \textbf{TS. Phạm Hồng Phong}, người đã tận tình hướng dẫn, góp ý và định hướng cho em trong suốt quá trình thực hiện đồ án tốt nghiệp. Những nhận xét và chỉ dẫn của thầy là cơ sở quan trọng giúp em hoàn thiện nội dung nghiên cứu, điều chỉnh cách trình bày báo cáo và cải thiện chất lượng triển khai hệ thống.
...
Tôi xin cam đoan đồ án tốt nghiệp với đề tài \textbf{“Xây dựng hệ thống quản lý dinh dưỡng cá nhân hóa”} là kết quả nghiên cứu và thực hiện của riêng tôi dưới sự hướng dẫn của \textbf{TS. Phạm Hồng Phong}.
...
MET & Metabolic Equivalent of Task \\[0.15cm]
MVC & Model View Controller \\[0.15cm]
NHLBI & National Heart, Lung, and Blood Institute \\[0.15cm]
ORM & Object Relational Mapping \\[0.15cm]
OWASP & Open Worldwide Application Security Project \\[0.15cm]
...
MET & Hệ số chuyển hóa tương đương của hoạt động thể chất \\[0.15cm]
EMA & Exponential Moving Average \\[0.15cm]
...
Xuất phát từ những vấn đề trên, đề tài \textbf{"Xây dựng hệ thống quản lý dinh dưỡng cá nhân hóa"} được thực hiện nhằm xây dựng một hệ thống hỗ trợ quản lý thông tin sức khỏe, theo dõi nhật ký dinh dưỡng và phân tích dữ liệu dựa trên các thuật toán được triển khai trong hệ thống. Ngoài các chức năng quản lý dữ liệu, đề tài tập trung nghiên cứu kiến trúc hệ thống, thiết kế cơ sở dữ liệu và xây dựng các thuật toán phục vụ tính toán nhu cầu năng lượng, đánh giá chất lượng thực phẩm, phân tích dữ liệu dinh dưỡng và hỗ trợ nhập liệu bằng công nghệ nhận dạng. Điểm nghiên cứu của đề tài tập trung vào việc thiết kế và triển khai các thuật toán phục vụ phân tích dữ liệu dinh dưỡng, góp phần nâng cao khả năng hỗ trợ theo dõi và đánh giá tình trạng dinh dưỡng của hệ thống.
...
\begin{figure}[htbp]
    ...
    \caption{Mô hình tổng quan dữ liệu của hệ thống quản lý dinh dưỡng cá nhân hóa}
...
Hệ thống sử dụng MySQL làm hệ quản trị cơ sở dữ liệu quan hệ và Sequelize ORM để ánh xạ giữa các bảng dữ liệu với các đối tượng trong mã nguồn. Sequelize được sử dụng để định nghĩa model, kiểu dữ liệu, ràng buộc, quan hệ giữa các bảng và thực hiện các truy vấn CRUD. Trong phiên bản mã nguồn hiện tại, ứng dụng xác thực kết nối và gọi cơ chế đồng bộ model của Sequelize khi khởi động; một số script riêng được sử dụng để khởi tạo hoặc bổ sung dữ liệu phục vụ quá trình phát triển. Cách tổ chức này giúp phần lớn thao tác dữ liệu được thực hiện thông qua ORM thay vì viết trực tiếp câu lệnh SQL trong các controller và service.
...
\textbf{Kiến trúc Frontend:} Frontend được tổ chức theo kiến trúc React SPA dạng component. Hệ thống điều hướng bằng React Router và bảo vệ quyền truy cập bằng Auth Context. Việc quản lý trạng thái dữ liệu (cache, đồng bộ) được thực hiện tối ưu qua TanStack Query, giảm thiểu gánh nặng gọi API lặp lại. Quá trình trao đổi dữ liệu với Backend do Axios đảm nhiệm. Kiến trúc Frontend chi tiết sẽ được trình bày tại Chương 3 (Hình \ref{fig:frontend_architecture}).

\textbf{Kiến trúc Backend:} Backend tổ chức theo mô hình MVC kết hợp tầng Service tập trung nghiệp vụ. Kiến trúc phân lớp Backend được trình bày chi tiết tại Hình \ref{fig:backend_architecture}. Controller chỉ nhận nhiệm vụ nhận và trả dữ liệu, mọi xử lý cốt lõi (tính toán dinh dưỡng, thuật toán, quét mã vạch) được trừu tượng hóa vào Service. Cách thiết kế này nâng cao tính độc lập của logic hệ thống, giúp dễ dàng mở rộng và bảo trì.

\textbf{Kiến trúc cơ sở dữ liệu:} Cơ sở dữ liệu MySQL được thiết kế quan hệ, tổ chức xoay quanh thông tin người dùng, lịch sử dinh dưỡng, cân nặng và luyện tập. Cơ sở dữ liệu được phân chia thành các nhóm dữ liệu người dùng, nhật ký, Meal Planner, Adaptive TDEE và Scanner; chi tiết phân nhóm và sơ đồ thực thể liên kết (ERD) được trình bày tại Mục 2.4. Việc ứng dụng Sequelize ORM giúp đảm bảo tính tương thích và bảo toàn dữ liệu bằng các ánh xạ Model, quản lý quan hệ khóa ngoại thay cho câu lệnh truy vấn SQL nguyên thủy.

\textbf{Đánh giá kiến trúc hệ thống:} Kiến trúc đa tầng được ứng dụng cung cấp độ gắn kết lỏng lẻo giữa Frontend và Backend, hỗ trợ tích hợp linh hoạt các luồng nghiệp vụ chính. Các thành phần có tính độc lập cao, vừa phục vụ tính năng phân tích dữ liệu hiệu quả, vừa đảm bảo khả năng triển khai hệ thống an toàn và mở rộng trong tương lai.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw=black, thick, fill=blue!5, rounded corners=4pt,
                    text width=4.2cm, align=center, font=\small\sffamily, inner sep=5pt,
                    minimum height=1.2cm},
        storebox/.style={rectangle, draw=gray!60, thick, fill=gray!8, rounded corners=3pt,
                    text width=3.8cm, align=center, font=\scriptsize\sffamily, inner sep=4pt, dashed},
        dbbox/.style={rectangle, draw=black, thick, fill=green!5, rounded corners=4pt,
                    text width=3.8cm, align=center, font=\small\sffamily, inner sep=5pt,
                    minimum height=1cm},
        req/.style={thick, -Latex, blue!80!black},
        res/.style={thick, -Latex, red!80!black, dashed},
        lreq/.style={font=\scriptsize\sffamily, text=blue!80!black, fill=white, inner sep=1.5pt},
        lres/.style={font=\scriptsize\sffamily, text=red!80!black, fill=white, inner sep=1.5pt},
        coltitle/.style={font=\large\bfseries\sffamily}
    ]

    % ═══════════════ BACKEND (cột giữa, anchor chính) ═══════════════
    \node[box] (api) {\textbf{RESTful API Route}\\[0.5ex]
        \scriptsize \texttt{/api/v1/*}};

    \node[box, below=of api] (mw) {\textbf{Middleware xác thực}\\[0.5ex]
        \scriptsize Kiểm tra Bearer Token};

    \node[box, below=of mw] (ctrl) {\textbf{Controller}\\[0.5ex]
        \scriptsize Validate, gọi Service};

    \node[box, below=of ctrl] (svc) {\textbf{Service}\\[0.5ex]
        \scriptsize Xử lý nghiệp vụ};

    % ═══════════════ FRONTEND (cột trái, cách xa 4.5cm) ═══════════════
    \node[box, fill=blue!10, left=4.5cm of api] (react)
        {\textbf{React Frontend}\\[0.5ex]
        \scriptsize Pages, Components, Modals};

    \node[box, below=of react] (query)
        {\textbf{TanStack Query}\\[0.5ex]
        \scriptsize Quản lý state, cache, invalidate};

    \node[box, below=of query] (axios)
        {\textbf{Axios Instance}\\[0.5ex]
        \scriptsize Gắn Bearer token vào header\\[0.3ex]
        \scriptsize Đọc token từ \texttt{localStorage}};

    \node[storebox, below=of axios] (ls)
        {\textbf{localStorage (Browser)}\\[0.3ex]
        \scriptsize Lưu \texttt{accessToken}};

    % ═══════════════ DATABASE (cột phải, cách xa 3.5cm) ═══════════════
    \node[dbbox, right=3.5cm of svc] (model)
        {\textbf{Sequelize Model}\\[0.3ex]
        \scriptsize ORM};

    \node[dbbox, below=of model] (db)
        {\textbf{MySQL Database}};

    % ═══════════════ TITLES (trên node cao nhất mỗi cột) ═══════════════
    \node[coltitle, above=0.8cm of api]   (t_be) {BACKEND};
    \node[coltitle] at (t_be -| react)    (t_fe) {FRONTEND};
    \node[coltitle] at (t_be -| model)    (t_db) {DATABASE};

    % ═══════════════ ĐƯỜNG PHÂN CÁCH CỘT ═══════════════
    \path (axios.east) -- (api.west) coordinate[midway] (s1);
    \path (api.east) -- (model.west) coordinate[midway] (s2);
    \draw[thick, dashed, gray!40] (s1 |- t_be.north) ++(0,0.3) -- (s1 |- db.south) ++(0,-0.3);
    \draw[thick, dashed, gray!40] (s2 |- t_be.north) ++(0,0.3) -- (s2 |- db.south) ++(0,-0.3);

    % ═══════════════ REQUEST (xanh, nét liền) ═══════════════
    % React → Query → Axios
    \draw[req] ([xshift=-4mm]react.south) -- ([xshift=-4mm]query.north);
    \draw[req] ([xshift=-4mm]query.south) -- ([xshift=-4mm]axios.north);

    % Axios ↔ localStorage
    \draw[req, gray!60] ([xshift=3mm]axios.south) -- node[right, font=\tiny\sffamily, text=gray] {ghi} ([xshift=3mm]ls.north);
    \draw[req, gray!60] ([xshift=-3mm]ls.north) -- node[left, font=\tiny\sffamily, text=gray] {đọc} ([xshift=-3mm]axios.south);

    % Axios → API Route (đường ngang dài, nhãn ở nhánh đầu)
    \draw[req] ([yshift=2mm]axios.east) -- node[lreq, above] {HTTP Request} ++(2.0,0) |- ([yshift=2mm]api.west);

    % Backend xuống: API → MW → Ctrl → Svc
    \draw[req] ([xshift=-4mm]api.south) -- ([xshift=-4mm]mw.north);
    \draw[req] ([xshift=-4mm]mw.south)  -- ([xshift=-4mm]ctrl.north);
    \draw[req] ([xshift=-4mm]ctrl.south) -- ([xshift=-4mm]svc.north);

    % Svc → Model → DB
    \draw[req] ([yshift=2mm]svc.east) -- node[lreq, above] {Query} ([yshift=2mm]model.west);
    \draw[req] ([xshift=-4mm]model.south) -- ([xshift=-4mm]db.north);

    % ═══════════════ RESPONSE (đỏ, đứt nét) ═══════════════
    % DB → Model
    \draw[res] ([xshift=4mm]db.north) -- ([xshift=4mm]model.south);

    % Model → Svc
    \draw[res] ([yshift=-2mm]model.west) -- node[lres, below] {Data} ([yshift=-2mm]svc.east);

    % Svc → Ctrl → MW → API
    \draw[res] ([xshift=4mm]svc.north)  -- node[lres, right] {Result} ([xshift=4mm]ctrl.south);
    \draw[res] ([xshift=4mm]ctrl.north) -- ([xshift=4mm]mw.south);
    \draw[res] ([xshift=4mm]mw.north)   -- ([xshift=4mm]api.south);

    % API → Axios (đường ngang dài, nhãn ở nhánh đầu)
    \draw[res] ([yshift=-2mm]api.west) -- node[lres, above] {Response JSON} ++(-2.0,0) |- ([yshift=-2mm]axios.east);

    % Axios → Query → React
    \draw[res] ([xshift=4mm]axios.north) -- ([xshift=4mm]query.south);
    \draw[res] ([xshift=4mm]query.north) -- ([xshift=4mm]react.south);
    ...
    \vspace{0.2cm}
    \small \textit{* Ghi chú: Các API dữ liệu cá nhân yêu cầu Access Token trong header. Nếu xác thực hợp lệ, request được chuyển qua Controller, Service và Model để truy vấn cơ sở dữ liệu; kết quả được trả về Frontend dưới dạng JSON thống nhất. Xử lý lỗi xác thực (401) và tự động cấp lại token được mô tả chi tiết tại Hình 2.4.}
\end{figure}
...
POST & \path{/api/v1/auth/login} & Đăng nhập và nhận Access Token \\
\hline
GET & \path{/api/v1/auth/me} & Lấy thông tin user hiện tại \\
\hline
POST & \path{/api/v1/diary/entries} & Thêm bản ghi nhật ký ăn uống \\
\hline
POST & \path{/api/v1/weight} & Ghi nhận cân nặng \\
\hline
POST & \path{/api/v1/adaptive-tdee/calculate} & Tính toán TDEE thích ứng \\
\hline
POST & \path{/api/v1/meal-planner/generate} & Tạo gợi ý thực đơn (Meal Planner) \\
\hline
POST & \path{/api/v1/scanner/ai-vision} & Nhận dạng thành phần từ ảnh (Scanner) \\
\hline
GET & \path{/api/v1/report/pdf} & Xuất báo cáo dinh dưỡng định dạng PDF \\
\hline
\end{longtable}
...
\begin{figure}[htbp]
    ...
    \node[layer, fill=red!8, draw=red!50, below=of md]
        (db) {MySQL Database\\
              {\footnotesize\normalfont Lưu trữ toàn bộ dữ liệu — kết nối qua Sequelize ORM}};
...
Một ví dụ là chức năng dashboard. Khi người dùng xem dữ liệu trong ngày, Backend không chỉ lấy nhật ký ăn uống mà còn cần tổng hợp năng lượng, protein, carbohydrate, lipid, chất xơ, nước uống, vận động và cân nặng. Các dữ liệu này được xử lý ở service rồi trả về cho Frontend dưới dạng cấu trúc thống nhất. Nếu toàn bộ xử lý đặt trong controller, mã nguồn sẽ khó bảo trì khi chức năng thay đổi. Các phép tính nghiệp vụ cụ thể, chẳng hạn ước tính năng lượng tiêu hao khi luyện tập theo MET, được triển khai tại tầng Service để tách khỏi xử lý HTTP; cơ sở tính toán và cách hiện thực được trình bày tại Mục~\ref{sec:met-exercise}.
...
\section{Các thuật toán xử lý chính}Chương 3 bên cạnh việc trình bày cách triển khai Backend và Frontend còn tập trung vào các thuật toán nghiệp vụ cốt lõi của hệ thống NutriTrack. Các nhóm xử lý được trình bày gồm Nutrition Core, ước tính năng lượng luyện tập theo MET, Adaptive TDEE, Food Scoring, Health Insights và Meal Planner. Mỗi nhóm đảm nhiệm một vai trò riêng trong quá trình tính toán và phân tích dữ liệu, đồng thời liên kết với nhau thông qua dữ liệu đầu vào và đầu ra của hệ thống.
...
BMI = W/H^2
\),
trong đó \(W\) là cân nặng tính bằng kilogram và \(H\) là chiều cao tính bằng mét; kết quả có đơn vị kg/m\(^2\). WHO sử dụng công thức này và mô tả BMI là một chỉ dấu thay thế cho mức độ mỡ cơ thể; các phép đo bổ sung như vòng eo có thể hỗ trợ quá trình đánh giá \cite{whoObesityOverweight2025}. Đối với người trưởng thành, bảng phân loại của NHLBI chia BMI thành bốn khoảng: dưới 18,5 là thiếu cân, từ 18,5 đến 24,9 là bình thường, từ 25,0 đến 29,9 là thừa cân và từ 30,0 trở lên là béo phì \cite{nhlbiPracticalGuide2000}. Trong NutriTrack, kết quả BMI được làm tròn đến một chữ số thập phân và ánh xạ thành một trong bốn nhãn trên để Frontend hiển thị thống nhất.

BMI được tính từ cân nặng và chiều cao nên không trực tiếp phân biệt khối mỡ với khối cơ. Chỉ số này có thể đánh giá cao mức độ mỡ ở người có khối cơ lớn và đánh giá thấp ở người bị mất khối cơ; mối quan hệ giữa BMI và lượng mỡ cơ thể cũng có thể thay đổi theo tuổi, giới tính và một số đặc điểm quần thể \cite{nhlbiPracticalGuide2000}. Vì vậy, NutriTrack chỉ sử dụng BMI để phân loại sơ bộ tình trạng cân nặng, không xem đây là kết luận chẩn đoán y khoa độc lập.
...
Sau khi ước lượng BMR, hệ thống tính TDEE tĩnh theo công thức \textbf{TDEE = BMR × PAL}. PAL, viết tắt của Physical Activity Level, biểu diễn tổng năng lượng tiêu hao trong 24 giờ dưới dạng bội số của BMR và được xác định về mặt khái niệm theo quan hệ \textbf{PAL = TEE/BMR}. Theo báo cáo FAO/WHO/UNU, khi biết BMR và PAL có thể ước lượng tổng nhu cầu năng lượng hằng ngày; PAL cần phản ánh lối sống hoạt động thường xuyên, bao gồm cả hoạt động nghề nghiệp và hoạt động trong thời gian rảnh \cite{fao_who_unu_2001}. Trong NutriTrack, mức vận động do người dùng khai báo được ánh xạ thành một hệ số triển khai để tạo staticTDEE ban đầu. Giá trị này là ước lượng tham khảo và có thể được thay thế bằng Adaptive TDEE khi hệ thống đã thu thập đủ dữ liệu thực tế. Phần tính toán Adaptive TDEE được tách riêng và trình bày tại Mục~\ref{sec:adaptive-tdee}.
...
\subsection{Ước tính năng lượng luyện tập theo MET}
\label{sec:met-exercise}

MET (Metabolic Equivalent of Task) biểu thị tỷ lệ giữa mức chuyển hóa khi thực hiện một hoạt động và mức chuyển hóa khi nghỉ. Một MET được quy ước tương đương 1~kcal/kg/giờ và xấp xỉ mức tiêu hao năng lượng khi ngồi yên \cite{paCompendiumWebsite}. Trong NutriTrack, giá trị MET được sử dụng để ước tính năng lượng tiêu hao của một buổi luyện tập dựa trên loại hoạt động, cân nặng và thời gian thực hiện.

\[
E = MET_s \times m \times \frac{t}{60}
\]

Trong đó, \(E\) là năng lượng tiêu hao ước tính, đơn vị kcal; \(MET_s\) là giá trị \texttt{defaultMet} của hoạt động được lấy từ \texttt{MET\_TABLE}; \(m\) là cân nặng người dùng tính bằng kg; \(t\) là thời gian luyện tập tính bằng phút. Hàm \texttt{calculateExerciseCalories} chuyển thời gian từ phút sang giờ, thực hiện phép tính và làm tròn kết quả đến một chữ số thập phân trước khi lưu vào trường \texttt{caloriesBurned} của \texttt{ExerciseLog}.

Compendium of Physical Activities cung cấp hệ thống mã và giá trị MET cho nhiều nhóm hoạt động với các mức cường độ khác nhau \cite{herrmann2024Compendium}. Dựa trên nguồn tham chiếu này, NutriTrack tổ chức \texttt{MET\_TABLE} theo các lựa chọn sử dụng trên giao diện. Một số hoạt động như đi bộ, chạy bộ, đạp xe và tập tạ được chia thành nhiều mức cường độ; các hoạt động còn lại sử dụng một giá trị mặc định đại diện. Frontend lấy danh sách hoạt động và mức năng lượng ước tính theo giờ từ Backend, sau đó chỉ gửi khóa hoạt động và thời gian luyện tập khi người dùng ghi nhận dữ liệu. Phép tính cuối cùng vẫn được thực hiện tập trung tại Backend.

Khi hồ sơ chưa có cân nặng, phiên bản hiện tại tạm sử dụng 60~kg để duy trì phép tính; đây là cơ chế dự phòng của NutriTrack, không phải giá trị do Compendium quy định. Các giá trị đang sử dụng là standard MET nên kết quả chỉ mang tính ước tính và có thể khác với năng lượng tiêu hao thực tế của từng cá nhân \cite{paCompendiumCorrectedMets}.

\subsection{Adaptive TDEE}
\label{sec:adaptive-tdee}
Adaptive TDEE là nhóm xử lý được xây dựng để điều chỉnh giá trị TDEE dựa trên dữ liệu thực tế của người dùng theo từng tuần. Trong Nutrition Core, TDEE được tính từ BMR và hệ số vận động. Cách tính này tạo ra giá trị nền ban đầu, tuy nhiên mô hình cân bằng năng lượng động cho thấy cần tính đến sự thích nghi của mức tiêu hao năng lượng trong quá trình thay đổi cân nặng; do đó, TDEE tĩnh không nhất thiết duy trì sự chính xác trong toàn bộ thời gian theo dõi \cite{hall2011_lancet}. Mối quan hệ giữa thâm hụt năng lượng và biến động cân nặng không hoàn toàn giống nhau; thành phần cơ thể ban đầu và tỷ lệ khối mỡ, khối không mỡ trong phần trọng lượng thay đổi có thể làm thay đổi năng lượng tương ứng với mỗi kilogram cân nặng giảm \cite{chow_hall_2008}. Vì vậy, hệ thống sử dụng TDEE tĩnh làm mốc ban đầu và bổ sung cơ chế Adaptive TDEE để hiệu chỉnh dần mức tiêu hao năng lượng ước lượng dựa trên nhật ký ăn uống và dữ liệu cân nặng.
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}p{4.8cm}|>{\raggedright\arraybackslash}p{5.5cm}|}
\caption{Các nhóm dữ liệu chính trong Food Scoring}
\label{tab:food_scoring_data} \\
\hline
\multicolumn{3}{|l|}{\textbf{(a) DỮ LIỆU ĐẦU VÀO}} \\
\hline
\textbf{Nhóm dữ liệu} & \textbf{Thành phần} & \textbf{Vai trò} \\
\hline
\endfirsthead
\caption[]{Các nhóm dữ liệu chính trong Food Scoring (tiếp theo)} \\
\hline
\endhead

Cơ sở chuẩn hóa & Năng lượng & Quy đổi các chất dinh dưỡng về mật độ trên 100 kcal. \\
\hline
Thành phần cộng điểm chính & Protein, chất xơ & Phản ánh mật độ protein và chất xơ. \\
\hline
Thành phần trừ điểm & Đường, natri & Kiểm soát các thành phần cần hạn chế. \\
\hline
Vi chất & Vitamin A, vitamin C, canxi, sắt & Bổ sung đánh giá về mật độ vi chất. \\
\hline
Dữ liệu phân loại & Loại thực phẩm, nhóm thực phẩm & Xử lý ngoại lệ theo đặc điểm của từng nhóm thực phẩm. \\
\hline
\multicolumn{3}{|l|}{\textbf{(b) QUY TẮC TÁC ĐỘNG ĐẾN ĐIỂM}} \\
\hline
\textbf{Thành phần} & \textbf{Điều kiện đánh giá} & \multicolumn{1}{c|}{\textbf{Mức tác động}} \\
\hline
Protein & Đạt mức tốt hoặc mức cao & \multicolumn{1}{c|}{+15 hoặc +25} \\
\hline
Chất xơ & Đạt mức tốt hoặc mức cao & \multicolumn{1}{c|}{+15 hoặc +25} \\
\hline
Đường & Vượt ngưỡng cảnh báo hoặc ngưỡng cao & \multicolumn{1}{c|}{-10 hoặc -25} \\
\hline
Natri & Vượt ngưỡng cảnh báo hoặc ngưỡng cao & \multicolumn{1}{c|}{-10 hoặc -25} \\
\hline
Vi chất & Đạt mức tốt hoặc mức cao & \multicolumn{1}{c|}{+5 hoặc +10} \\
\hline
\multicolumn{3}{|l|}{\textbf{(c) PHÂN LOẠI KẾT QUẢ}} \\
\hline
\multicolumn{1}{|c|}{\textbf{Khoảng điểm}} & \multicolumn{1}{c|}{\textbf{Mức phân loại}} & \textbf{Ý nghĩa} \\
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{4.2cm}|}{Từ 80 điểm trở lên} & \multicolumn{1}{>{\centering\arraybackslash}p{4.8cm}|}{Lành mạnh} & Mật độ dinh dưỡng tốt trong phạm vi thuật toán. \\
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{4.2cm}|}{Từ 60 đến dưới 80 điểm} & \multicolumn{1}{>{\centering\arraybackslash}p{4.8cm}|}{Khá tốt} & Có thành phần có lợi và ít yếu tố bất lợi. \\
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{4.2cm}|}{Từ 40 đến dưới 60 điểm} & \multicolumn{1}{>{\centering\arraybackslash}p{4.8cm}|}{Trung bình} & Cần cân nhắc theo khẩu phần và mục tiêu cá nhân. \\
\hline
\multicolumn{1}{|>{\centering\arraybackslash}p{4.2cm}|}{Dưới 40 điểm} & \multicolumn{1}{>{\centering\arraybackslash}p{4.8cm}|}{Cần hạn chế} & Có dấu hiệu nghèo dinh dưỡng hoặc có nhiều thành phần cần kiểm soát. \\
\hline
\end{longtable}
...
Food Scoring bổ sung một thước đo định lượng về chất lượng thực phẩm cho NutriTrack. Trong nhật ký ăn uống, điểm thực phẩm có thể giúp người dùng nhận biết món nào có mật độ dinh dưỡng tốt hơn. Trong Health Insights, kết quả này có thể hỗ trợ phát hiện khẩu phần có nhiều thực phẩm điểm thấp. Trong Meal Planner, điểm thực phẩm có thể được dùng như một tín hiệu phụ khi lựa chọn hoặc thay thế nguyên liệu, bên cạnh các ràng buộc chính như năng lượng mục tiêu, protein, carbohydrate, chất béo và cấu hình bữa ăn. Toàn bộ quy trình xử lý chấm điểm thực phẩm được mô tả trong Hình \ref{fig:food_scoring_flow}.

\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[
        node distance=0.8cm,
        box/.style={rectangle, draw=blue!80!black, thick, rounded corners, minimum width=8cm, minimum height=1cm, align=center, fill=blue!5},
        arrow/.style={thick, ->, >=stealth, draw=gray!80!black}
        ]
        
        \node (step1) [box] {Lấy dữ liệu thực phẩm};
        \node (step2) [box, below=of step1] {Kiểm tra dữ liệu hợp lệ};
        \node (step3) [box, below=of step2] {Chuẩn hóa trên 100 kcal};
        \node (step4) [box, below=of step3] {Cộng điểm cho protein, chất xơ và vi chất};
        \node (step5) [box, below=of step4] {Trừ điểm cho đường và natri};
        \node (step6) [box, below=of step5] {Áp dụng ngoại lệ và giới hạn điểm};
        \node (step7) [box, below=of step6] {Trả về điểm, nhãn và tooltip cho Frontend};
    ...
    \end{tikzpicture}
    \caption{Sơ đồ luồng Food Scoring}
    \label{fig:food_scoring_flow}
\end{figure}
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{5.5cm}|>{\raggedright\arraybackslash}p{6cm}|}
\caption{Cơ chế phân tầng trong Health Insights}
\label{tab:health_insights_layers} \\
\hline
\multicolumn{3}{|l|}{\textbf{(a) CƠ CHẾ TRIAGE THEO MỨC CALO}} \\
\hline
\multicolumn{1}{|c|}{\textbf{Tầng xử lý}} & \textbf{Điều kiện theo calo} & \textbf{Cảnh báo thiếu hụt được ưu tiên} \\
\hline
\endfirsthead
\caption[]{Cơ chế phân tầng trong Health Insights (tiếp theo)} \\
\hline
\endhead

\multicolumn{1}{|c|}{Sinh tồn} & Dưới 50\% mục tiêu & Chỉ cảnh báo lượng ăn quá thấp. \\
\hline
\multicolumn{1}{|c|}{Đa lượng} & Từ 50\% đến dưới 70\% mục tiêu & Cảnh báo thiếu calo, protein và chất béo. \\
\hline
\multicolumn{1}{|c|}{Vi lượng} & Từ 70\% mục tiêu trở lên & Kiểm tra chất xơ, canxi, sắt, vitamin C và vitamin A. \\
\hline
\multicolumn{3}{|l|}{\textbf{(b) PHÂN LOẠI SEVERITY CỦA INSIGHT}} \\
\hline
\multicolumn{1}{|c|}{\textbf{Severity}} & \textbf{Vai trò hiển thị} & \textbf{Ý nghĩa} \\
\hline
\multicolumn{1}{|c|}{danger} & Ưu tiên hiển thị trước & Cảnh báo nguy cơ hoặc vấn đề cần được chú ý trước. \\
\hline
\multicolumn{1}{|c|}{warning} & Nhắc người dùng điều chỉnh & Cảnh báo cần chú ý. \\
\hline
\multicolumn{1}{|c|}{water} & Theo dõi lượng nước trong ngày & Cảnh báo hoặc nhắc nhở liên quan đến nước uống. \\
\hline
\multicolumn{1}{|c|}{suggestion} & Đưa ra hướng điều chỉnh nhẹ & Gợi ý cải thiện. \\
\hline
\end{longtable}
...
Kết quả thành công của Meal Planner gồm trạng thái xử lý, danh sách nguyên liệu được lựa chọn, đối tượng dữ liệu thực phẩm tương ứng và khối lượng tính bằng gram của từng nguyên liệu. Mã nguồn Backend trả danh sách này trong trường data và có thể kèm theo các cảnh báo về khối lượng quá nhỏ hoặc quá lớn. Từ thông tin dinh dưỡng của từng thực phẩm và khối lượng được tính toán, tầng hiển thị hoặc các bước xử lý tiếp theo có thể xác định giá trị dinh dưỡng ước tính của khẩu phần. Phiên bản hiện tại chưa trả về một đối tượng tổng hợp riêng chứa tổng năng lượng và tổng macro của toàn bộ bữa ăn. Nếu không tìm được tổ hợp khả thi, kết quả chứa thông tin lỗi hoặc gợi ý để người dùng thay đổi lựa chọn.
...
Mật khẩu người dùng không được lưu trực tiếp dưới dạng văn bản gốc. Khi đăng ký, Backend sử dụng thư viện \texttt{bcryptjs} để thực hiện thuật toán bcrypt với cost factor 12 trước khi lưu giá trị băm vào cơ sở dữ liệu. Khi đăng nhập, mật khẩu đầu vào được kiểm tra bằng hàm \texttt{bcrypt.compare}. OWASP khuyến nghị sử dụng các hàm băm mật khẩu chậm và thích ứng, trong đó Argon2id được ưu tiên cho hệ thống mới; đối với bcrypt, tài liệu nêu work factor tối thiểu là 10 \cite{owaspPasswordStorage}. Vì vậy, cost factor 12 của NutriTrack cao hơn mức tối thiểu được OWASP nêu cho bcrypt, nhưng không nên được mô tả là lựa chọn hiện đại được OWASP ưu tiên nhất.
...
Bên cạnh xác thực, hệ thống áp dụng giới hạn request cho các route đăng nhập và đăng ký. Các route đăng nhập và đăng ký được giới hạn tối đa 20 request trong khoảng 15 phút theo địa chỉ IP. Đây là một hình thức throttling nhằm hạn chế số lượng request tự động trong thời gian ngắn \cite{owaspAuthentication}. Tuy nhiên, cơ chế hiện tại chỉ đếm theo IP và chưa gắn bộ đếm với từng tài khoản, do đó được xem là một lớp phòng vệ bổ sung chứ không phải biện pháp ngăn chặn hoàn toàn brute-force hoặc credential stuffing. Ngoài ra, chức năng AI Vision trong Scanner cũng có giới hạn request riêng theo người dùng, nhằm hạn chế việc gửi quá nhiều ảnh trong thời gian ngắn. Ở phía Frontend, Access Token có thời hạn 15 phút được lưu trong \texttt{localStorage} và được Axios interceptor đọc để gắn vào header \texttt{Authorization}. Cách lưu trữ này thuận tiện cho việc duy trì trạng thái đăng nhập và tự động gửi lại request sau khi làm mới token, nhưng \texttt{localStorage} luôn có thể được JavaScript cùng origin truy cập. OWASP cảnh báo rằng dữ liệu nhạy cảm hoặc định danh phiên trong \texttt{localStorage} có thể bị lấy cắp khi ứng dụng xuất hiện lỗ hổng XSS \cite{owaspHTML5Security}. Vì vậy, việc bảo vệ Refresh Token bằng HttpOnly Cookie không loại bỏ hoàn toàn rủi ro đối với Access Token đang được lưu tại phía client.
...
\hline
Refresh Token & Làm mới phiên đăng nhập & HttpOnly Cookie, SameSite=Strict, Secure ở production \\
\hline
requireAuthApi & Bảo vệ API nghiệp vụ & Kiểm tra token trước khi vào controller \\
\hline
bcrypt & Bảo vệ mật khẩu người dùng & Băm bằng bcryptjs với cost factor 12 \\
\hline
Rate limit & Hạn chế request bất thường & 20 request/15 phút/IP cho login và register \\
\hline
...
Chương này cũng đã trình bày các thành phần xử lý cốt lõi và chức năng hỗ trợ nhập liệu của NutriTrack. Mỗi thành phần giải quyết một vấn đề riêng trong quá trình quản lý và phân tích dữ liệu dinh dưỡng. Vai trò và các điểm kỹ thuật nổi bật của các thành phần được tổng hợp trong Bảng~\ref{tab:tong-hop-thanh-phan-cot-loi}.

\begin{table}[htbp]
    \centering
    \caption{Tổng hợp các thành phần xử lý cốt lõi của NutriTrack}
    \label{tab:tong-hop-thanh-phan-cot-loi}
    \renewcommand{\arraystretch}{1.2}
    \setlength{\tabcolsep}{4pt}
    \small
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{0.2\textwidth}|>{\raggedright\arraybackslash}p{0.3\textwidth}|>{\raggedright\arraybackslash}X|}
        \hline
        \textbf{Thành phần} & \textbf{Vấn đề giải quyết} & \textbf{Điểm kỹ thuật nổi bật} \\
        \hline
        Nutrition Core & Tính toán thống nhất các chỉ số và mục tiêu dinh dưỡng nền tảng. & Cung cấp calculateAllMetrics và các kết quả dùng chung cho Dashboard, Meal Planner và PDF Report. \\
        \hline
        Adaptive TDEE & TDEE tĩnh không phản ánh đầy đủ biến động theo dữ liệu thực tế của người dùng. & EMA với hệ số 0,1; warm-up 14 ngày; rolling tối đa bốn tuần; clamp trong khoảng 70–130\% TDEE tĩnh; kiểm tra điều kiện dữ liệu. \\
        \hline
        Food Scoring & Tổng năng lượng không phản ánh đầy đủ chất lượng dinh dưỡng của thực phẩm. & Chuẩn hóa thành phần theo 100 kcal; cơ chế cộng và trừ điểm; giới hạn điểm cứng theo chất xơ, vi chất và natri. \\
        \hline
        Health Insights & Dữ liệu nhật ký khó diễn giải và có thể tạo cảnh báo thiếu hụt không phù hợp với thời điểm. & Triage theo mức đạt calo; Context-Awareness theo thời gian và số bữa; phân loại mức độ ưu tiên của insight. \\
        \hline
        Meal Planner & Tính khẩu phần thủ công khó xử lý sự chồng lấn macro giữa nhiều nguyên liệu. & Cố định phần rau; lập hệ phương trình tuyến tính \(3 \times 3\); khử Gauss kết hợp chọn phần tử trụ từng phần; kiểm tra tính khả thi của nghiệm. \\
        \hline
        Hybrid Nutrition Scanner & Nhập liệu sản phẩm đóng gói mất thời gian và dữ liệu cục bộ có thể chưa đầy đủ. & Tra cứu Local Verified/Unverified, Open Food Facts và Gemini AI Vision; Physics Validation; cập nhật confidenceScore từ dữ liệu đóng góp. \\
        \hline
    \end{tabularx}
\end{table}

Các thành phần trên không hoạt động độc lập mà liên kết với nhau thông qua dữ liệu mục tiêu cá nhân, nhật ký ăn uống, cân nặng, nước uống và thông tin thực phẩm. Frontend tiếp nhận và trình bày kết quả xử lý thông qua các màn hình như Dashboard, Diary, Meal Planner và Scanner. Bên cạnh đó, chức năng xuất báo cáo PDF tổng hợp dữ liệu theo khoảng thời gian nhằm phục vụ quá trình theo dõi và đánh giá.
...
\hline
Meal Planner & Mục tiêu năng lượng, macro và cấu hình bữa ăn & Sinh khẩu phần phù hợp với mục tiêu & Trả về nguyên liệu, khối lượng gram và cảnh báo về khẩu phần \\
\hline
...
Đối với Adaptive TDEE, thực nghiệm được thực hiện trên dữ liệu nhật ký năng lượng và cân nặng được tổ chức theo từng tuần. Thuật toán không sử dụng trực tiếp chênh lệch giữa hai lần cân thô làm đầu vào cuối cùng. Backend truy vấn thêm dữ liệu cân nặng trong giai đoạn trước tuần hiện tại để khởi tạo quá trình làm mượt, sau đó áp dụng Exponential Moving Average với hệ số 0,1 để xác định cân nặng đầu tuần, cân nặng cuối tuần và mức thay đổi cân nặng đã làm mượt. Một tuần chỉ được xử lý khi có ít nhất năm ngày ghi nhận năng lượng và tối thiểu hai bản ghi cân nặng hợp lệ. TDEE tính theo tuần được kiểm soát trong khoảng từ 70\% đến 130\% TDEE tĩnh trước khi tham gia tính rolling TDEE. Giá trị rolling được tổng hợp từ tuần hiện tại và tối đa ba tuần hợp lệ trước đó có cùng mục tiêu; User.adaptiveTDEE chỉ được cập nhật khi đã có ít nhất hai tuần hợp lệ. Kết quả thực nghiệm cho thấy hệ thống thực hiện đầy đủ các bước kiểm tra dữ liệu, làm mượt cân nặng, kiểm soát biên và lưu lịch sử tính toán theo tuần.
...
Đối với Meal Planner, thực nghiệm tập trung vào khả năng xác định khối lượng nguyên liệu dựa trên mục tiêu macro của bữa ăn. Khi tìm được tổ hợp hợp lệ, Backend trả về danh sách nguyên liệu cùng khối lượng tính bằng gram và các cảnh báo liên quan đến tính khả thi của khẩu phần. Dữ liệu dinh dưỡng của từng nguyên liệu có thể được sử dụng để ước tính giá trị dinh dưỡng của kết quả ở tầng hiển thị hoặc trong bước xử lý tiếp theo. Khi tổ hợp nguyên liệu không tạo được nghiệm hợp lệ, hệ thống trả về thông báo để người dùng thay đổi lựa chọn thay vì ghi nhận một khẩu phần không khả thi. Đối với Hybrid Nutrition Scanner, hai luồng chính được kiểm tra gồm tra cứu barcode và nhận dạng ảnh nhãn dinh dưỡng. Khi barcode khớp với dữ liệu đã có, hệ thống trả về thông tin sản phẩm tương ứng. Khi dữ liệu được trích xuất từ ảnh còn thiếu hoặc không đạt điều kiện kiểm tra hợp lệ, hệ thống yêu cầu xác nhận trước khi lưu vào cơ sở dữ liệu.
...
\bibitem{paCompendiumWebsite}
Compendium of Physical Activities,
``Compendium of Physical Activities -- Quantifying Physical Activity Energy Expenditure'',
\textit{\url{https://pacompendium.com/}},
truy cập ngày 14/07/2026.

\bibitem{paCompendiumCorrectedMets}
Compendium of Physical Activities,
``Corrected METS -- Adults'',
\textit{\url{https://pacompendium.com/corrected-mets/}},
truy cập ngày 14/07/2026.
...
\bibitem{herrmann2024Compendium}
Herrmann, S. D., Willis, E. A., Ainsworth, B. E., et al. (2024),
``2024 Adult Compendium of Physical Activities: A third update of the energy costs of human activities'',
\textit{Journal of Sport and Health Science},
Tập 13, Số 1, Trang 6--12.
DOI: \url{https://doi.org/10.1016/j.jshs.2023.10.010}.
...
\bibitem{nhlbiPracticalGuide2000}
...
\bibitem{whoObesityOverweight2025}
