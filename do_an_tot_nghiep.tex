...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % --- CỘT 1 ---
        \node[usecase] (uc_register) at (-4.5, 0) {Đăng ký tài khoản};
        \node[usecase] (uc_profile) at (-4.5, -1.5) {Khởi tạo / cập nhật\\hồ sơ sức khỏe};
        \node[usecase] (uc_log_water) at (-4.5, -3.0) {Ghi nhận nước uống};
        \node[usecase] (uc_exercise) at (-4.5, -4.5) {Ghi nhận luyện tập};
        \node[usecase] (uc_dashboard) at (-4.5, -6.5) {Xem Dashboard};
        \node[usecase] (uc_report) at (-4.5, -8.25) {Xuất báo cáo PDF};
        \node[usecase] (uc_meal) at (-4.5, -10.5) {Xây dựng thực đơn};
        ...
        \node[usecase] (uc_scanner) at (0, -9.25) {Sử dụng Hybrid\\Nutrition Scanner};

        % --- CỘT 3 ---
        \node[usecase] (uc_search) at (4.5, -2.25) {Tìm kiếm / bổ sung\\thực phẩm};
        \node[usecase] (uc_analysis) at (4.5, -6.5) {Xem phân tích\\dinh dưỡng};
        \node[usecase] (uc_barcode) at (4.5, -8.5) {Quét mã vạch};
        \node[usecase] (uc_ocr) at (4.5, -10.0) {Nhận dạng bảng\\dinh dưỡng};
        ...
        % --- ACTORS ---
        \node[actor] (user) at (-8.5, -4.5) {Người dùng};
        \pic at ([yshift=0.8cm]user.center) {stick figure};

        \node[actor, draw=black, thick, fill=gray!10, minimum width=2.2cm, minimum height=0.8cm, align=center, rounded corners=5pt] (openfood) at (9.5, -8.5) {<<system>>\\Open Food Facts\\API};
        \node[actor, draw=black, thick, fill=gray!10, minimum width=2.2cm, minimum height=0.8cm, align=center, rounded corners=5pt] (gemini) at (9.5, -10.0) {<<system>>\\Gemini AI\\Vision};

        % --- INCLUDE ARROWS ---
        \draw[include] (uc_log_food) -- node[above, font=\scriptsize] {<<include>>} (uc_search);
        \draw[include] (uc_dashboard) -- node[above, font=\scriptsize, pos=0.4] {<<include>>} (uc_analysis);
        
        \draw[include] (uc_dashboard) -- node[below, sloped, font=\scriptsize, pos=0.6] {<<include>>} (uc_aggregate);
        \draw[include] (uc_analysis) -- node[below, sloped, font=\scriptsize, pos=0.6] {<<include>>} (uc_aggregate);
        \draw[include] (uc_report) -- node[above, sloped, font=\scriptsize, pos=0.6] {<<include>>} (uc_aggregate);
        
        % --- EXTEND ARROWS ---
        \draw[extend] (uc_barcode) -- node[above, sloped, font=\scriptsize] {<<extend>>} (uc_scanner);
        \draw[extend] (uc_ocr) -- node[below, sloped, font=\scriptsize] {<<extend>>} (uc_scanner);
        ...
        % --- USER CONNECTIONS ---
        \draw[-, thick] (user.east) -- (-7.0, -4.5);
        \draw[-, thick] (-7.0, 0.75) -- (-7.0, -10.5);

        % Nối vào Cột 1
        \foreach \y in {0, -1.5, -3.0, -4.5, -6.5, -8.25, -10.5} {
            \draw[-, thick] (-7.0, \y) -- (-6.2, \y);
        }
        % Nối vào Cột 2
        \foreach \y in {0.75, -0.75, -2.25, -3.75, -9.25} {
            \draw[-, thick] (-7.0, \y) -- (-1.7, \y);
        }
