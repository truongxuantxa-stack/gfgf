...
Tôi xin cam đoan đồ án tốt nghiệp với đề tài \textbf{“Xây dựng hệ thống quản lý dinh dưỡng cá nhân”} là kết quả nghiên cứu và thực hiện của riêng tôi dưới sự hướng dẫn của \textbf{TS. Phạm Hồng Phong}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        storebox/.style={rectangle, draw=gray!60, thick, fill=gray!8, rounded corners=3pt,
                    text width=3.8cm, align=center, font=\scriptsize\sffamily, inner sep=5pt, dashed},
        dbbox/.style={rectangle, draw=black, thick, fill=green!5, rounded corners=4pt,
                    text width=3.8cm, align=center, font=\small\sffamily, inner sep=6pt},
        req/.style={thick, -Latex, blue!80!black},
        res/.style={thick, -Latex, red!80!black, dashed},
        err/.style={thick, -Latex, red!60!black, dashed},
        ...
        label_err/.style={font=\scriptsize\sffamily, text=red!60!black, fill=white, inner sep=2pt}
    ]

    % ── BACKEND (đặt trước để làm anchor) ───────────────────────────────────────
    \node[box] (api) {\textbf{RESTful API Route}\\[1ex]
        \scriptsize \texttt{/api/v1/auth},\\[0.3ex]
        \texttt{/api/v1/diary/entries},\\[0.3ex]
        \texttt{/api/v1/dashboard}, \ldots};
    \node[box, below=0.8cm of api]       (middleware) {\textbf{Middleware xác thực}\\[1ex]
        \scriptsize Kiểm tra \texttt{Authorization: Bearer <token>}\\[0.4ex]
        Nếu sai $\rightarrow$ trả \textbf{401 JSON}};
    \node[box, below=0.8cm of middleware] (controller) {\textbf{Controller}\\[1ex]
        \scriptsize Nhận request, validate, gọi Service, trả response};
    \node[box, below=0.8cm of controller] (service) {\textbf{Service}\\[1ex]
        \scriptsize Xử lý nghiệp vụ, tính toán, tổng hợp dữ liệu};

    % ── FRONTEND ─────────────────────────────────────────────────────────────────
    % Axios ngang hàng với Controller (giữa backend)
    \node[box, left=2.8cm of controller] (axios) {\textbf{Axios / TanStack Query}\\[1ex]
        \scriptsize Gắn \texttt{Bearer token} vào header\\[0.3ex]
        Gửi HTTP request, quản lý cache\\[0.3ex]
        Response Interceptor: bắt 401 $\to$ tự gọi \texttt{/auth/refresh-token}};
    \node[box, fill=blue!10, above=0.8cm of axios] (react) {\textbf{React Frontend}\\[1ex]
        \scriptsize Pages, Components, Forms, Modals};
    % localStorage node — bên dưới Axios
    \node[storebox, below=0.8cm of axios] (localstorage) {%
        \textbf{localStorage (Browser)}\\[0.5ex]
        \scriptsize Lưu \texttt{accessToken}\\[0.2ex]
        {\color{gray}(xóa khi 401 không refresh được)}};

    % ── DATABASE ─────────────────────────────────────────────────────────────────
    % Sequelize Model và MySQL ngang hàng với Service
    \node[dbbox, right=2.8cm of service]   (model) {\textbf{Sequelize Model}\\[0.5ex]
        \scriptsize ORM — ánh xạ SQL $\leftrightarrow$ JS object};
    \node[dbbox, below=0.8cm of model]     (db)    {\textbf{MySQL Database}\\[0.5ex]
        \scriptsize Lưu trữ dữ liệu người dùng,\\nhật ký, thực phẩm, \ldots};

    % ── TITLES ───────────────────────────────────────────────────────────────────
    \node[font=\large\bfseries\sffamily, above=1.3cm of react]      (title_fe) {FRONTEND};
    \node[font=\large\bfseries\sffamily] at (title_fe -| api)        (title_be) {BACKEND};
    \node[font=\large\bfseries\sffamily] at (title_fe -| model)      (title_db) {DATABASE};

    % ── SEPARATORS ───────────────────────────────────────────────────────────────
    \path (axios.east) -- (api.west)   coordinate[midway] (sep1);
    \path (api.east)   -- (model.west) coordinate[midway] (sep2);
    \draw[thick, dashed, gray!40] (sep1 |- title_fe.north) -- (sep1 |- db.south);
    \draw[thick, dashed, gray!40] (sep2 |- title_be.north) -- (sep2 |- db.south);

    % ── REQUEST FLOWS (Xanh đậm) ─────────────────────────────────────────────────
    % React → Axios
    \draw[req] ([xshift=-0.4cm]react.south) -- ([xshift=-0.4cm]axios.north);
    % Axios → localStorage (ghi token)
    \draw[req, gray!60] (axios.south) -- node[right, font=\scriptsize\sffamily, text=gray] {lưu token} (localstorage.north);
    % Axios → API Route
    \draw[req] ([yshift=0.25cm]axios.east)
        -- node[label_req, above] {HTTP Request (Bearer token)}
        ++(1.4,0) |- (api.west);
    % API Route → Middleware → Controller → Service
    \draw[req] ([xshift=-0.45cm]api.south)        -- ([xshift=-0.45cm]middleware.north);
    \draw[req] ([xshift=-0.45cm]middleware.south)  -- ([xshift=-0.45cm]controller.north);
    \draw[req] ([xshift=-0.45cm]controller.south)  -- ([xshift=-0.45cm]service.north);
    % Service → Sequelize Model → MySQL
    \draw[req] ([yshift=0.2cm]service.east)
        -- node[label_req, above] {Query}
        ++(1.2,0) |- ([yshift=0.2cm]model.west);
    \draw[req] ([xshift=-0.4cm]model.south) -- ([xshift=-0.4cm]db.north);

    % ── RESPONSE FLOWS (Đỏ đứt nét) ──────────────────────────────────────────────
    % MySQL → Sequelize Model
    \draw[res] ([xshift=0.4cm]db.north) -- ([xshift=0.4cm]model.south);
    % Sequelize Model → Service
    \draw[res] ([yshift=-0.2cm]model.west)
        -- node[label_res, below] {Data}
        ++(-0.8,0) |- ([yshift=-0.2cm]service.east);
    % Service → Controller → Middleware → API Route
    \draw[res] ([xshift=0.45cm]service.north)    -- node[label_res, right] {Result} ([xshift=0.45cm]controller.south);
    \draw[res] ([xshift=0.45cm]controller.north) -- ([xshift=0.45cm]middleware.south);
    \draw[res] ([xshift=0.45cm]middleware.north) -- ([xshift=0.45cm]api.south);
    % API Route → Axios (response JSON thành công)
    \draw[res] ([yshift=-0.25cm]api.west)
        -- node[label_res, above] {Response JSON}
        ++(-1.4,0) |- ([yshift=-0.25cm]axios.east);
    % Axios → React
    \draw[res] ([xshift=0.4cm]axios.north) -- ([xshift=0.4cm]react.south);

    % ── LUỒNG 401 (Middleware → Axios interceptor bắt) ───────────────────────────
    % 401 JSON từ Middleware đi theo tuyến response riêng, Axios interceptor bắt lại
    \draw[err] (middleware.west)
        -- node[label_err, above] {401 JSON}
        ++(-0.9,0) |- ([yshift=0.35cm]axios.east);

    % ── GHI CHÚ localStorage (đọc token) ─────────────────────────────────────────
    \draw[req, gray!60] (localstorage.east) -| ([xshift=-0.4cm, yshift=0.25cm]axios.east);
