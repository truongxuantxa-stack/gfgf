...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Backend
        \node[box, fill=orange!10, right=3.5cm of react] (express) {\textbf{Express}\\(Controller)};
        \node[box, fill=orange!10, right=0.8cm of express] (service) {\textbf{Service}\\(Business Logic)};
        \node[box, fill=orange!10, right=0.8cm of service] (sequelize) {\textbf{Sequelize\\ORM}};

        % Backend Group
        \begin{scope}[on background layer]
            \node[rectangle, draw=black, fill=yellow!10, inner sep=15pt, 
                  fit=(express) (service) (sequelize)] (backend_box) {};
            \node[grouptitle, above=0.2cm of backend_box.north] (backend_title) {BACKEND (Node.js)};
        \end{scope}

        % Database
        \node[box, fill=green!10, right=1.5cm of sequelize] (mysql) {\textbf{MySQL}\\(Database)};

        % External Services
        \node[extbox, fill=gray!10, above=1.5cm of express] (jwt) {JWT\\Authentication};
        \node[extbox, fill=gray!10, above=1.5cm of service] (openfood) {Open Food Facts\\API};
        \node[extbox, fill=gray!10, above=1.5cm of sequelize] (gemini) {Gemini AI\\Vision};

        % Connections
        \draw[arrow] (react) -- node[above, font=\bfseries\fontsize{10pt}{12pt}\selectfont] {RESTful API} (express);
        \draw[arrow] (express) -- (service);
        \draw[arrow] (service) -- (sequelize);
        \draw[arrow] (sequelize) -- (mysql);

        \draw[arrow, dashed, <->] (express) -- (jwt);
        \draw[arrow, dashed, <->] (service) -- (openfood);
        \draw[arrow, dashed, <->] (service) -- (gemini);
