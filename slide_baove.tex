...
\begin{frame}{Adaptive TDEE – Điều chỉnh theo dữ liệu thực tế}
    ...
    \begin{tikzpicture}[>=Stealth, node distance=0.3cm and 0.4cm,
        ...
        \node[box, right=of w1] (ema) {EMA làm mượt chuỗi cân nặng};
        \node[box, right=of ema] (agg) {Tổng hợp theo tuần};
        \node[box, below=0.5cm of agg] (est) {Ước lượng TDEE tuần};
        \node[box, left=of est] (roll) {Trung bình trượt 4 tuần và kiểm tra giới hạn};
    ...
    \begin{center}
        ...
        \vspace{0.15cm}
        \small \textit{\textcolor{NutriDark}{TDEE tĩnh là điểm khởi đầu; Adaptive TDEE điều chỉnh dần theo dữ liệu thực tế của người dùng.}}
    \end{center}
    
    \vspace{0.1cm}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textcolor{NutriDark}{\textbf{\modulelabel{TDEE tĩnh} -- Ước tính từ công thức}}
            \begin{itemize}
                \item \small TDEE = BMR $\times$ PAL.
                \item \small BMR và mức hoạt động đều là giá trị ước tính.
                \item \small Phù hợp làm giá trị khởi tạo khi chưa có dữ liệu cá nhân.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textcolor{NutriDark}{\textbf{\modulelabel{Adaptive TDEE} -- Hiệu chỉnh từ thực tế}}
            \begin{itemize}
                \item \small Dựa trên năng lượng nạp vào và xu hướng cân nặng.
                \item \small Suy ngược mức tiêu hao trung bình theo thời gian.
                \item \small Làm mượt và kiểm soát trước khi cập nhật.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Phân tích chất lượng dinh dưỡng}
    \begin{columns}[T]
        ...
        \begin{column}{0.44\textwidth}
            ...
            \begin{tikzpicture}[>=Stealth, node distance=0.3cm,
                ...
                \node[box, below=of warn] (score) {Daily Health Score\\(Điểm sức khỏe trong ngày)};
                \draw[arrow] (log) -- (cmp);
                \draw[arrow] (cmp) -- (warn);
                \draw[arrow] (warn) -- (score);
            \end{tikzpicture}
        \end{column}
    \end{columns}
    ...
    \vspace{0.3cm}
    \centering
    \scriptsize \textit{*Food Scoring đánh giá từng thực phẩm; Daily Health Score đánh giá tổng thể dữ liệu dinh dưỡng trong ngày.}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            ...
            \begin{tikzpicture}[>=Stealth, node distance=0.5cm,
                ...
                arrow/.style={->, NutriDark}]
                \node[box] (select) {Chọn mẫu/tổ hợp nguyên liệu};
