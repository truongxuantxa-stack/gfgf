...
% Sơ đồ được vẽ bằng draw.io, lưu tại images/hybrid_scanner_flow.drawio
% Để cập nhật sơ đồ: mở file .drawio → chỉnh sửa → File > Export As > PDF
% (bật checkbox "Crop") → lưu đè vào images/hybrid_scanner_flow.pdf
% ─────────────────────────────────────────────────────────────────────────
\begin{frame}{Hybrid Nutrition Scanner}
    \textbf{Ưu tiên dữ liệu cục bộ trước, chỉ gọi nguồn bên ngoài khi cần.}
    \vspace{0.1cm}
    \begin{center}
        \includegraphics[width=0.93\textwidth, height=0.72\textheight, keepaspectratio]{images/hybrid_scanner_flow.pdf}
    \end{center}
\end{frame}
