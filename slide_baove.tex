...
\begin{frame}{Kiến trúc tổng thể của hệ thống}
    ...
    \begin{tikzpicture}[>=Stealth, node distance=0.6cm and 0.4cm,
        ...
        \node[layer, below=1cm of api, xshift=-4.35cm] (router) {Router};
        \node[layer, right=0.4cm of router] (controller) {Controller};
        \node[layer, right=0.4cm of controller] (service) {Service};
        \node[layer, right=0.4cm of service] (model) {Sequelize Model};
        ...
        \node[layer, dashed, right=1.5cm of react] (ext) {Dịch vụ ngoài (AI Vision, Open Food Facts)};
        
        \draw[arrow] (client) -- (react);
        \draw[arrow] (react) -- node[right, font=\footnotesize] {Axios} (api);
        \draw[arrow] (api) -| (router);
        \draw[arrow] (router) -- node[above, font=\footnotesize] {Middleware} (controller);
