...
\begin{frame}{BACKUP B1.4: Xử lý ngoại lệ (Edge Cases)}
    ...
    \begin{itemize}
        \small
        \item \textbf{Nghiệm âm} ($w_i < 0$): Phân loại là \textbf{Error} (Lỗi), kết quả không hợp lệ. Xảy ra khi một nguyên liệu "đóng góp ngược".
        \item \textbf{Dưới 10g (trừ fat) / Trên 500g (trừ rau):} Phân loại là \textbf{Warning} (Cảnh báo). Hệ thống vẫn có thể trả về \texttt{success: true} nếu có warning.
        \item \textbf{Cơ chế Retry:} Tối đa 15 lần với các tổ hợp thực phẩm thay thế, hệ thống sẽ chọn lại \texttt{bestAttempt} là phương án có số lỗi mức \textbf{Error} ít nhất.
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{BACKUP B2.2: Weighted Penalty — Hàm phạt theo mức sai lệch}
    \textbf{2. Weighted Penalty — Hàm phạt phân đoạn theo mức độ sai lệch}
    \vspace{0.1cm}
...
\begin{frame}[fragile]{BACKUP B3.4: Guardrails \& Phát hiện Plateau \& Cron Job}
    ...
    \begin{block}{\scriptsize Logic phát hiện Plateau}
        ...
        $\bullet$ \textbf{Hành động 2:} Tính mục tiêu calo mới chặn dưới bằng BMR.\\
        $\bullet$ \textbf{Hành động 3:} Gửi thông báo: "Cơ thể đang thích ứng chuyển hóa".
    \end{block}
    
    \vspace{0.1cm}
    \textbf{9. Tự động hóa (Cron Job):} Chạy ngầm vào 0:00 sáng mỗi Thứ Hai để tổng hợp dữ liệu dinh dưỡng/cân nặng của toàn bộ người dùng trong tuần cũ, từ đó tự động tính toán và cập nhật TDEE cho tuần mới.
\end{frame}
...
% --- BACKUP B6 ---
\begin{frame}[label=b6]{BACKUP B6.1: Cơ chế Scanner \& Trust Score (Crowdsourcing)}
\hypertarget{b6_target}{}
    \textbf{1. Physics Validation (Xác thực vật lý)}
    \begin{itemize}
        \item Đảm bảo dữ liệu nhãn thành phần dinh dưỡng hợp lý: Tổng khối lượng các chất (Protein + Carbs + Fat + $\dots$) không được vượt quá 100g.
        \item Lượng năng lượng (kcal) phải tính toán khớp hoặc xấp xỉ với macro (1g Protein/Carbs $\approx$ 4 kcal, 1g Fat $\approx$ 9 kcal).
    \end{itemize}
    
    \vspace{0.2cm}
    \textbf{2. Trust Score \& Crowdsourcing (Cơ chế đóng góp cộng đồng)}
    \begin{itemize}
        \item \textbf{Một đóng góp:} Khởi tạo dữ liệu với mức độ tin cậy (\textit{confidence}) là $0.3$.
        \item \textbf{Hai đóng góp tương đồng:} Hệ thống tự động nâng \textit{confidence} lên $0.5$.
        \item \textbf{Từ ba đóng góp tương đồng:} \textit{Confidence} đạt $0.8$ và sản phẩm có thể được chuyển sang trạng thái \textbf{Verified} (Đã xác minh).
        \item \textbf{Xử lý báo cáo sai phạm:} Khi người dùng báo cáo dữ liệu sai, sản phẩm chuyển sang trạng thái \textbf{Disputed} (Tranh chấp) để quản trị viên kiểm tra.
    \end{itemize}
\end{frame}
