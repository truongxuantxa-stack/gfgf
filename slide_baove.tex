...
\begin{frame}{Kết quả thực nghiệm và kiểm thử}
    ...
    \begin{center}
        ...
        \begin{tabularx}{0.98\textwidth}{>{\hsize=0.28\hsize}X | >{\hsize=0.72\hsize}X}
            ...
            Adaptive TDEE & Test 7 ngày: TDEE tĩnh 2500 $\rightarrow$ Adaptive 2350 kcal (−6\%); phát hiện plateau, đề xuất giảm mục tiêu. \\
...
\begin{frame}{BACKUP B1.3: Ưu điểm của phương pháp Khử Gauss}
    ...
    \begin{itemize}
        \item \textbf{Tốc độ cực nhanh (Thời gian hằng số):} Với số ẩn cố định $n = 3$, độ phức tạp $O(n^3)$ thực chất trở thành $O(1)$. Hệ thống có thể tính toán hàng ngàn tổ hợp trong nháy mắt (Real-time).
        \vspace{0.2cm}
...
\begin{frame}{BACKUP B4.2: Quan hệ \& Điểm thiết kế nổi bật}
    ...
    \begin{itemize}
        \item \textbf{User (1) $\rightarrow$ (N):} DiaryEntry, WeightLog, WaterLog, ExerciseLog, UserMealConfig, AdaptiveTDEELog, ProductContribution.
