...
\begin{frame}{BACKUP B5.3: Phân tích chất lượng bữa ăn (Habit Extraction)}
    \textbf{4. Luồng phân tích và trích xuất thói quen}
    
    \vspace{0.1cm}
    \begin{center}
    \begin{tikzpicture}[>=Stealth,
        box/.style={draw=NutriDark, fill=NutriLightGray, rounded corners, inner sep=3pt, text width=9cm, text centered, font=\scriptsize},
        arrow/.style={->, thick, NutriDark}]
        
        \node[box, fill=NutriGreen!20] (n1) {Danh sách nhật ký ăn uống (diaryEntries)};
        \node[box, below=0.12cm of n1] (n2) {Gọi hàm \texttt{buildWeeklyFoodReport(diaryEntries, range)}};
        \node[box, below=0.12cm of n2] (n3) {Gộp các mục ăn uống (entry) theo từng món ăn};
        \node[box, below=0.12cm of n3] (n4) {Chấm điểm từng món bằng hàm \texttt{scoreFoodItem()}};
        \node[box, below=0.12cm of n4] (n5) {Sắp xếp món có điểm thấp lên trước để ưu tiên cảnh báo};
        \node[box, below=0.12cm of n5] (n6) {Tìm thói quen tốt / thói quen cần hạn chế};
        \node[box, below=0.12cm of n6] (n7) {Tính thống kê mức độ an toàn (đỏ / vàng / xanh)};
        \node[box, below=0.12cm of n7, fill=NutriGreen!20] (n8) {\textbf{Kết luận chất lượng ăn uống tuần/tháng}};
